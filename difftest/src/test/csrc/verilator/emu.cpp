...
inline void Emulator::single_cycle() {
  ...
  bool in_range  = (begin <= cycles) && (cycles <= end) && enable_waveform;
  if (in_range) tfp->dump(cycles * 2);
  dut_ptr->clock = 0;
  dut_ptr->eval();
  ...
  if (in_range) tfp->dump(cycles * 2 + 1);
  dut_ptr->clock = 1;
  dut_ptr->eval();

  // if (!(cycles & 1023)) printf("Cycle=%lu\r", cycles);
...
inline char* Emulator::timestamp_filename(time_t t, char *buf) {
  ...
  snprintf(buf_time, 64, "%ld", t);
  char *noop_home = getenv("FST_HOME");
  if (noop_home != NULL)
  {
    int len = snprintf(buf, 1024, "%s/%s", noop_home, buf_time);
    return buf + len;
  }
  noop_home = getenv("NOOP_HOME");
...
inline char* Emulator::waveform_filename(time_t t) {
  ...
  char *p = timestamp_filename(t, buf);
  strcpy(p, ".fst");
  printf("dump wave %lu-%lu to %s...\n", args.log_begin, args.log_end, buf);
  return buf;
}
